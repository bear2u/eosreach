...
class DelegateBandwidthListViewModel @Inject internal constructor(
    ...
    override fun filterIntents(intents: Observable<DelegateBandwidthListIntent>): Observable<DelegateBandwidthListIntent> = Observable.merge(
        intents.ofType(DelegateBandwidthListIntent.Init::class.java).take(1),
        intents.filter {
            !DelegateBandwidthListIntent.Init::class.java.isInstance(it)
        }
    )
